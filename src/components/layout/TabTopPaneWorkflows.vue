...
#top-pane-workflows
  ...
  .left-content
    ...
    ul.page-list
      li(
        v-for="(workflow, idx) in focusWorkflows",
        :class="{ 'selected': idx === focusWorkflowIndex }"
        @click="() => focusWorkflowClicked(idx)"
      )
        .name {{ workflow.name }}
        .highlight-bar
        .v-divider
...
<script>
  ...
  export default {
    setup() {
      let focusWorkflowIndex = ref(0)

      const focusWorkflowClicked = (idx) => {
        focusWorkflowIndex.value = idx
      }

      const event = reactive({
        profile: computed(() => store.getters["user/sessionUser"]?.profile),
        focusWorkflows: computed(() => store.getters["user/focusProject"]?.workflows),
      })

      return { ...toRefs(event), focusWorkflowClicked, focusWorkflowIndex }
    },
  }
</script>
