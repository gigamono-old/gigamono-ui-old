...
import { convertRemtoPxInt } from "@/application/utils/css"
import { getCSSVar } from "@/application/utils/css"

// Handles onDrop event on the container element.
const onDrop = (event, containerElement) => {
  const steps = store.getters["workflow/workspaceProjectWorkflow"].steps
  ...
  store.dispatch("workflow/addStepToWorkspaceProjectWorkflow", dropPoints)
...
const updateContainerBounds = (latestCursorDropPoint, steps, containerElement) => {
  const containerBounds = store.getters["workflow/workspaceProjectWorkflow"].containerBounds
  ...
  store.dispatch("workflow/updateContainerBounds", newContainerBounds)
...
// TODO: Use translate3D for scrolling and compare with native scroll
const onScroll = () => {}
